...
import { computeRouteDistanceKm, computeRouteProgressKm, normalizeRouteAddress } from "../lib/routeDistance";
import fs from "fs";
...
import {
  ...
  patchAsaasChargeInvoiceMirrors,
  persistAsaasChargeInvoice,
} from "../lib/persistAsaasChargeInvoice";
import {
  getWhatsappProvider,
...
async function resolvePrincipal(token: string): Promise<ResolvedPrincipal | null> {
  ...
  try {
    const sb = createSupabaseAdminClient();
    if (!sb) return null;
    // Abort real — Promise.race sem signal deixa fetch zumbi e a Vercel segura a resposta.
...
<div class="container">
  ...
  // ── Migration: histórico/versionamento de Plano de Ação DHL ──
  try {
    await supabaseAdmin.rpc('exec_sql', { sql: `
    ...
    `});
    console.log('[Migration] Tabela dhl_occurrence_reports verificada/criada.');
  } catch (e: any) {
    console.log('[Migration] dhl_occurrence_reports:', e.message || 'ok');
  }
...

{
  ...
  app.post("/api/asaas/create-charge", requireAuth, requireRole('administrador', 'diretoria', 'financeiro'), async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (cleanLookup) {
        // Lookup local com AbortSignal — Promise.race sem abort deixa fetch zumbi na Vercel.
      ...
      if (charges && Array.isArray(charges) && charges.length > 0) {
        ...
        for (let i = 0; i < validCharges.length; i++) {
          ...
          // Database-first no split: persiste e segue — PIX/NF em background após a resposta.
          ...
          console.log(`[Asaas] Cobrança split criada+persistida: ${payment.id} | ${charge.name || clientName} | CNPJ: ${cleanCnpj} | R$ ${charge.value}`);
      ...
      const t0 = Date.now();
      // Ref. interna de rastreio (TMSEG-…) — vem do frontend ou gera no servidor.
      const trackingRef = String(invoiceNumber || '').trim() || `TMSEG-${Date.now()}`;
      // Deadline global: cancela I/O pendente para a Vercel liberar o HTTP (sem zumbi).
      ...
      try {
        console.log(`[ASAAS-EMISSAO] 1. Iniciando processamento... client=${clientName || '-'} valor=${amountNum} venc=${dueDate} empresa=${issuerCompany || '-'} ref=${trackingRef}`);
        ...
        // PASSO 1 — Cliente Asaas (AbortSignal cancela fetch de verdade)
        console.log('[ASAAS-EMISSAO] 2. Buscando/Criando cliente...');
        let customer: any;
        try {
          // Sem postalCode: evita PUT updateCustomerAddress (2ª chamada Asaas).
        ...
        // PASSO 3 — Supabase (service role) — teto 5s com abortSignal
        ...
        // PASSO 4 — HTTP 200 imediato. NF NÃO roda aqui (trabalho pós-res.json
  ...
  /** Progresso A→atual→B via Directions API (origem, posição atual, destino). */
  app.get('/api/route-progress', requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      if (!result.success) {
        return res.json({ success: false, error: result.error || 'ROTA_TOTAL_INDISPONIVEL' });
      }

      const etaMinutes = result.etaMinutes;
