 ...
 */
export function throttleZapiSend<T>(label: string, fn: () => Promise<T>): Promise<T> {
  const prev = chain;
  let release!: () => void;
  ...
  return (async () => {
    ...
    if (wait > 0) {
      console.log(`[Z-API Fila] Aguardando ${(wait / 1000).toFixed(1)}s antes de enviar (${label}) — espaçamento anti-spam.`);
      await sleep(wait);
    }
    const p = fn();
    // Libera o próximo da fila quando este envio TERMINAR (sucesso ou erro)
    // ou após o teto MAX_HOLD_MS — uma conexão pendurada não trava a fila.
