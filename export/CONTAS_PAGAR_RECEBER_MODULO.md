...
- **Cores**: Altere as cores do Tailwind conforme sua identidade visual (ex: `text-red-700` → `text-blue-700`)
- **Entidades**: Se não usar tabelas `clients` e `providers`, ajuste o `fetchEntities` no formulário
- **Autenticação**: O módulo lê `localStorage.getItem('userData')` para o nome do usuário. Adapte conforme seu sistema de auth
- **Data mínima**: Remova o `.gte('due_date', '...')` se quiser carregar todos os registros desde sempre
