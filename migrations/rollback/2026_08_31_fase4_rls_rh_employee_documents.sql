-- ROLLBACK Fase 4 RH RLS — public.rh_employee_documents.
...
BEGIN
  ...
  IF policy_total = 0 THEN
    EXECUTE
      'CREATE POLICY "Allow all for rh_employee_documents" '
  ...
  IF policy_total <> 1 OR expected_policy_total <> 1 THEN
    RAISE EXCEPTION
      'Drift no rollback de public.rh_employee_documents: total=%, esperada=%',
      policy_total,
