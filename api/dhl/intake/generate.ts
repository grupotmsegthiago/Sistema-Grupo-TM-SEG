...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (!provider) {
      res.status(404).json({
        error: `Fornecedor "${mission.provider}" não localizado no cadastro.`,
        providerName: mission.provider,
      });
      return;
    }
    ...
    if (existing && notExpired) {
      if (existing.auto_reminders_paused_at) {
        await sb.from('dhl_supplier_intakes')
          .update({ auto_reminders_paused_at: null, auto_reminders_paused_by: null })
          .eq('id', existing.id);
        intake = { ...existing, auto_reminders_paused_at: null, auto_reminders_paused_by: null };
      }
    } else {
      ...
      if (iErr) {
        ...
        res.status(500).json({ error: 'Erro ao registrar intake: ' + (msg || 'falha desconhecida') });
        return;
      }
      intake = inserted;
    }
    ...
    res.status(200).json({
      ...
      emailSkipped,
      whatsappSent: false,
      whatsappError: wantsWhatsapp && !providerPhone ? 'Fornecedor sem telefone cadastrado' : null,
      whatsappSkipped: !wantsWhatsapp,
      channel,
