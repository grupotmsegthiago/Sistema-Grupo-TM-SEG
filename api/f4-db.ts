...
import { runF4DbOperation } from '../lib/f4ApiOperations.js';
import {
  f4QueryValue,
...
type Dependencies = {
  ...
  runOperation: typeof runF4DbOperation;
};
...
const defaults: Dependencies = {
  ...
  runOperation: runF4DbOperation,
};

export async function handleF4DbRequest(
  req: F4Request,
  res: F4Response,
  deps: Dependencies = defaults,
): Promise<void> {
  const op = f4QueryValue(req, 'op');
  const expectedMethod = op === 'capacity' ? 'GET' : op === 'vacuum' ? 'POST' : '';
  if (!expectedMethod) {
  ...
  sendF4Result(
    res,
    await deps.runOperation(op as 'capacity' | 'vacuum', req.body, admin),
  );
}

export default async function handler(req: F4Request, res: F4Response) {
  await handleF4DbRequest(req, res);
}
