...
/** Lista transferências (escopo de saque). 403 = chave sem permissão de saque via API. */
async function probeTransfersApi(
  apiKey: string,
  sandbox: boolean,
  ...
  const base = sandbox ? 'https://sandbox.asaas.com/api/v3' : 'https://api.asaas.com/v3';
  try {
    const res = await fetch(`${base}/transfers?limit=1`, {
      headers: { access_token: apiKey, 'Content-Type': 'application/json' },
    });
    const data = await res.json().catch(() => ({}));
    if (res.ok) return { ok: true };
...
function transferHint(transfersOk?: boolean, transfersError?: string): string | null {
  if (transfersOk === true) return null;
  if (transfersOk === false) {
    if (/saque via API|insufficient_permission/i.test(String(transfersError || ''))) {
      return (
        'Chave sem permissão de saque — no painel Asaas: Mecanismos de segurança (webhook transfer-approval) ' +
        '+ gerar NOVA chave em Chaves de API e atualizar na Vercel (redeploy).'
      );
    }
    return `GET /transfers falhou: ${transfersError || 'erro'}`;
  }
  return null;
}
...
export default async function handler(req: any, res: any) {
  ...
  const [invGestao, invSeg, invSec, trGestao, trSeg, trSec] = await Promise.all([
    ...
    probeTransfersApi(getAsaasApiKeyTmGestao(), env.tmGestao.sandbox),
    probeTransfersApi(getAsaasApiKeyTmSeguranca(), env.tmSeguranca.sandbox),
    probeTransfersApi(getAsaasApiKeyTmSecurity(), env.tmSecurity.sandbox),
  ]);
  ...
  const report = {
    tmGestao: {
      ...
      transfersProbe: trGestao,
      hint: keyShapeHint(env.tmGestao, invGestao.ok) || transferHint(trGestao.ok, trGestao.error),
    },
    tmSeguranca: {
      ...env.tmSeguranca,
      invoicesProbe: invSeg,
      transfersProbe: trSeg,
      hint: keyShapeHint(env.tmSeguranca, invSeg.ok) || transferHint(trSeg.ok, trSeg.error),
    },
    tmSecurity: {
      ...env.tmSecurity,
      invoicesProbe: invSec,
      transfersProbe: trSec,
      hint: keyShapeHint(env.tmSecurity, invSec.ok) || transferHint(trSec.ok, trSec.error),
    },
  };
  ...
  res.status(200).json({
    ...
    fix:
      ...
      'Se transfersProbe falhar com “saque via API”, regenere a chave após ativar Mecanismos de segurança → webhook transfer-approval.',
  });
}
