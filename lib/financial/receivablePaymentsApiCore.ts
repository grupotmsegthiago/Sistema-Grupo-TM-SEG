/**
 * SSOT das operações de financial_transaction_payments.
 * Mesma regra do client legado — só muda o transporte (service_role no backend).
 */
import { computePaymentSettlement, type ReceivedPaymentInput } from './partialPayments.js';
...
export function isMissingTableError(err: QueryError): boolean {
  if (!err) return false;
  const msg = String(err.message || '');
  return err.code === '42P01' || err.code === 'PGRST205' || /financial_transaction_payments|schema cache/i.test(msg);
}

export function isMissingColumnError(err: QueryError): boolean {
  if (!err) return false;
  const msg = String(err.message || '');
  return err.code === '42703' || /amount_paid|amount_open|column/i.test(msg);
}

export function splitTitleNotes(notes: string | null | undefined): {
  text: string;
...
export function createReceivablePaymentsOps(client: PaymentsClient) {
  ...
  async function probePaymentsTable(): Promise<boolean> {
    ...
    const { error } = await client.from('financial_transaction_payments').select('id').limit(1);
  ...
  async function updateTransactionSettlement(params: {
    ...
    let { error } = await client.from('financial_transactions').update(base).eq('id', params.transactionId);
    ...
    if (error && isMissingColumnError(error)) {
      ...
      const retry = await client.from('financial_transactions').update(slim).eq('id', params.transactionId);
  ...
  async function listPaymentsForTransaction(
    transactionId: string,
    titleNotes?: string | null,
  ...
  async function addPaymentToTransaction(params: AddPaymentParams): Promise<{
  ...
  async function deletePaymentFromTransaction(params: DeletePaymentParams): Promise<{
    paid: number;
    open: number;
    status: string;
    notes?: string;
    ...
    if (useTable && paymentsTableAvailable !== false) {
      const { error } = await client.from('financial_transaction_payments').delete().eq('id', params.paymentId);
      if (error) {
        if (!isMissingTableError(error)) throw error;
        paymentsTableAvailable = false;
      } else {
