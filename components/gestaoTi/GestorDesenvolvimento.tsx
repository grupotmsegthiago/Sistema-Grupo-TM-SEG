...
function monitoringTone(status: string): HealthTone {
  ...
  return 'gray'; // unmonitored — nunca verde
}
...
export const GestorDesenvolvimento: React.FC = () => {
  ...
  const overall = health.length ? overallToneFromHealth(health) : 'gray';
  const critical = listCriticalConnections('p1');
  const dupSsot = listDuplicatedSsot();
  const unmonitored = listUnmonitoredModules();
  const openIncidents = incidents.filter((i) => i.state === 'aberto' || i.severity === 'P0' || i.severity === 'P1');

  return (
    <div
      className="space-y-4 pb-16 bg-slate-50/60 p-2 rounded-2xl animate-fade-in"
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-3 px-2 pt-2">
        <div>
          <div className="flex items-center gap-2 text-sky-800">
            ...
            <h1 className="text-xl font-black tracking-tight">Gestor de Desenvolvimento</h1>
          </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-8 gap-2" data-testid="gestor-desenvolvimento-kpis">
        <Kpi label="Saúde geral" value={overallLabel(overall)} tone={overall} />
        ...
        <Kpi label="Críticas (P0–P1)" value={String(critical.length)} tone="yellow" />
        ...
        <Kpi label="Incidentes abertos/P0–P1" value={String(openIncidents.length)} tone={openIncidents.length ? 'red' : 'green'} />
        <Kpi label="Mapa" value={GESTAO_TI_CATALOG_VERSION} tone="blue" />
      </div>

      <div className="px-2 text-[11px] text-slate-500" data-testid="gestor-desenvolvimento-build">
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        <Card title="Health checks existentes" icon={<Activity className="w-4 h-4 text-emerald-700" />} testId="gestor-desenvolvimento-health">
          <ul className="space-y-2">
            ...
            {health.map((h) => (
              <li key={h.id} className="border border-gray-100 rounded-lg px-3 py-2 text-xs">
                <div className="flex items-center justify-between gap-2">
                  <span className="font-semibold text-slate-800">{h.label}</span>
                  <ToneBadge tone={h.tone}>
                    {h.tone === 'green' ? 'saudável' : h.tone === 'yellow' ? 'atenção' : h.tone === 'red' ? 'falha' : h.tone === 'gray' ? 'sem dado' : 'info'}
                  </ToneBadge>
                </div>
                <div className="text-slate-500 mt-1 font-mono truncate">{h.path}</div>
                <div className="text-slate-600 mt-0.5">{h.summary || '—'}</div>
                <div className="text-[10px] text-slate-400 mt-1">
                  {h.latencyMs != null ? `${h.latencyMs} ms` : '—'} · retries {h.retries}
                  {h.statusCode != null ? ` · HTTP ${h.statusCode}` : ''}
                </div>
              </li>
            ))}
          </ul>
        ...
        <Card title="Central de incidentes (derivados)" icon={<AlertTriangle className="w-4 h-4 text-amber-700" />} testId="gestor-desenvolvimento-incidents">
          ...
          <ul className="space-y-2 max-h-[420px] overflow-auto">
            {incidents.slice(0, 40).map((inc) => (
              <li key={inc.code} className="border border-gray-100 rounded-lg px-3 py-2 text-xs">
                <div className="flex flex-wrap items-center gap-2 justify-between">
                  <span className="font-bold text-slate-800">{inc.code}</span>
                  <span className="font-mono text-[10px] px-1.5 py-0.5 rounded bg-slate-100">{inc.severity}</span>
                </div>
                <div className="mt-1 font-medium text-slate-700">{inc.title}</div>
                <div className="mt-1 text-slate-500">Módulo: {inc.moduleId}</div>
                <div className="mt-1 text-slate-600 whitespace-pre-wrap break-words">{inc.evidence}</div>
                <div className="mt-1 text-[10px] text-slate-400">
                  {inc.state} · qtd {inc.count} · {inc.firstSeenAt}
                </div>
                <div className="mt-1 text-slate-600">Impacto: {inc.impact}</div>
                <button
                  type="button"
                  disabled
          ...
          <table className="min-w-full text-[11px]">
            <thead className="text-left text-slate-500 border-b">
              <tr>
                ...
                <th className="py-2 pr-2">Monitor.</th>
              </tr>
            </thead>
            <tbody>
              {catalog.connections.map((c) => (
                <tr key={c.id} className="border-b border-gray-50 align-top">
                  ...
                  <td className="py-2 pr-2">
                    ...
                    <div className="text-slate-400 mt-0.5">{c.rule}</div>
                  </td>
                  <td className="py-2 pr-2">{c.type}</td>
                  <td className="py-2 pr-2 max-w-[220px]">{c.officialSource}</td>
                  <td className="py-2 pr-2 uppercase font-bold">{c.criticality}</td>
                  <td className="py-2 pr-2">
                    <ToneBadge tone={monitoringTone(c.monitoringStatus)}>{c.monitoringStatus}</ToneBadge>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        <Card title="Módulos catalogados" icon={<Server className="w-4 h-4 text-slate-700" />} testId="gestor-desenvolvimento-modules">
          <ul className="space-y-2">
            {catalog.modules.map((m) => (
              <li key={m.id} className="flex items-start justify-between gap-2 border border-gray-100 rounded-lg px-3 py-2 text-xs">
                <div>
                  <div className="font-bold text-slate-800">{m.name}</div>
                  <div className="text-slate-500">{m.domain} · {m.screens.join(', ') || '—'}</div>
                </div>
                <ToneBadge tone={monitoringTone(m.monitoringStatus)}>{m.monitoringStatus}</ToneBadge>
              </li>
            ))}
          </ul>
        </Card>
        ...
        <Card title="Fonte única da verdade" icon={<BookOpen className="w-4 h-4 text-violet-700" />} testId="gestor-desenvolvimento-ssot">
          <ul className="space-y-2 max-h-[480px] overflow-auto">
            {catalog.ssot.map((s) => (
              <li key={s.id} className="border border-gray-100 rounded-lg px-3 py-2 text-xs">
                <div className="flex items-center justify-between gap-2">
                  <span className="font-bold text-slate-800">{s.dataLabel}</span>
                  <ToneBadge
                    tone={
                ...
                <div className="mt-1 text-slate-600">Oficial: {s.officialSource}</div>
                <div className="mt-1 text-slate-500">Writers: {s.writers.join(' · ')}</div>
                <div className="text-slate-500">Readers: {s.readers.join(' · ')}</div>
                <div className="text-slate-500">Recalc: {s.recalculators.join(' · ') || '—'}</div>
                <div className="mt-1 text-amber-800">Risco divergência: {s.divergenceRisk}</div>
                <div className="mt-1 text-[10px] text-slate-400 font-mono">{s.evidence.join(' | ')}</div>
              </li>
            ))}
          </ul>
        </Card>
...
function Kpi({ label, value, tone }: { label: string; value: string; tone: HealthTone }) {
  return (
    <div className={`rounded-xl border px-3 py-2 ${TONE_CLASS[tone]}`}>
      ...
      <div className="text-sm font-black truncate mt-0.5" title={value}>
        {value}
      </div>
    </div>
  );
...
function overallLabel(tone: HealthTone): string {
  switch (tone) {
    case 'green':
      return 'Saudável';
    case 'yellow':
