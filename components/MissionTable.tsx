...
🗾 *LINK DO GOOGLE:* ${mission.mapLink || 'N/A'}`;
    ...
    const confirmDelete = async () => { if (!missionToDelete) return; setIsDeleting(true); try { await supabase.from('missions').update({ status: 'Cancelada' }).eq('id', missionToDelete.id); await logAction('UPDATE', 'Mission', missionToDelete.id, `Missão cancelada por ${currentUser?.name}`); showNotification('Sucesso', 'Missão cancelada com sucesso. O registro permanece no banco.', 'success'); setIsDeleteModalOpen(false); setMissionToDelete(null); fetchMissions(true); } catch (error: any) { showNotification('Erro', error.message, 'error'); } finally { setIsDeleting(false); } };
