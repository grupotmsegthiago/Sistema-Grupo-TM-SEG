...
import { Users, Briefcase, Circle, Wifi, WifiOff } from 'lucide-react';
...
function statusStyle(status: PresenceServiceStatus, isOnline: boolean): StatusStyle {
  switch (status) {
    case 'em_servico':
      return { border: 'border-green-300', dot: 'bg-green-500', text: 'text-green-700' };
    case 'em_almoco':
      return { border: 'border-red-300', dot: 'bg-red-500', text: 'text-red-600' };
    case 'aguardando_ponto':
      return { border: 'border-amber-300', dot: 'bg-amber-500', text: 'text-amber-700' };
    case 'online':
      return { border: 'border-sky-300', dot: 'bg-sky-500', text: 'text-sky-700' };
    default:
      return {
...
const PresenceUserCard: React.FC<UserCardProps> = ({ user, isOnline }) => {
  ...
  return (
    <div
      className={`group relative flex min-w-0 w-full flex-col items-center px-0.5 transition-opacity ${isOnline ? '' : 'opacity-55'}`}
    >
      ...
      >
        <p className="mb-1 font-black uppercase text-[9px] text-indigo-200">{displayName}</p>
        <pre className="whitespace-pre-wrap font-sans text-[10px] text-gray-100">{tooltip}</pre>
      </div>
    </div>
  );
};
...
const PresenceColumn: React.FC<PresenceColumnProps> = ({
  title,
  icon,
  users,
  isOnlineFn,
  ...
  <div className="min-w-0 flex-1">
    <div
      className={`mb-2 flex items-center gap-1.5 text-[10px] font-black uppercase tracking-wide ${accentClass}`}
    >
      {icon}
      <span>
        {title} · {users.length}
    ...
    {users.length === 0 ? (
      <p className="text-[10px] font-medium text-gray-400">{emptyLabel}</p>
    ) : (
...
const MissionTeamPresenceBoard: React.FC<Props> = ({ enabled = true }) => {
  ...
  const isUserOnline = useMemo(
    () => (user: PresenceUserState) => onlineIds.has(normalizePresenceUserId(user.userId)),
    [onlineIds]
  );
  ...
  const emServicoCount = useMemo(
    () =>
      displayUsers.filter(
        (u) => getPresenceServiceStatus(u, { isOnline: isUserOnline(u) }) === 'em_servico'
      ).length,
    [displayUsers, isUserOnline]
  );

  const onlineOnBoard = useMemo(
    () => displayUsers.filter((u) => isUserOnline(u)).length,
    [displayUsers, isUserOnline]
  );
  ...
  const grouped = useMemo(() => {
    ...
    > = {
      ...
      administrativo: { online: [], offline: [] },
      comercial: { online: [], offline: [] },
    };

    const byCategory: Record<PresenceCategory, PresenceUserState[]> = {
      operacao: [],
      administrativo: [],
      comercial: [],
    };
    ...
    for (const user of displayUsers) {
      byCategory[getPresenceCategory(user.role)].push(user);
    }

    for (const key of PRESENCE_CATEGORY_ORDER) {
      groups[key] = partitionPresenceBoardUsers(byCategory[key], isUserOnline);
    }
      ...
      {displayUsers.length === 0 ? (
        ...
        <div className="space-y-5">
          {PRESENCE_CATEGORY_ORDER.map((category) => {
            const { online, offline } = grouped[category];
            const total = online.length + offline.length;
            return (
              <div key={category} className="border-t border-gray-100 pt-4 first:border-t-0 first:pt-0">
                <div className="mb-3 flex items-center gap-1.5 text-[10px] font-black uppercase tracking-wide text-gray-500">
                  <span>
                    {PRESENCE_CATEGORY_LABELS[category]} · {total}
                  </span>
                </div>
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:gap-6">
                  <PresenceColumn
                    ...
                    icon={<Wifi size={11} />}
                    users={online}
                    isOnlineFn={isUserOnline}
                    emptyLabel="Ninguém conectado nesta categoria."
                  ...
                  <PresenceColumn
                    ...
                    users={offline}
                    isOnlineFn={isUserOnline}
                    emptyLabel="Todos conectados nesta categoria."
