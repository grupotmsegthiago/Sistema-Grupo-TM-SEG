...
-   **Financial Calculation Consistency:** Both the spreadsheet comparison table and the financial modal use `calculateMissionFinancials` for non-approved missions. Only `billing_approved=true` missions use stored `revenue_value`/`cost_value`. This ensures values match between the comparison view and the modal.
