...
-   **Atomic Location Persistence:** When updating mission location (Ponto B), `UpdateMissionModal` captures address + mapLink + coordinates atomically. If the agent pastes a Google Maps link, `reverseGeocode` resolves the textual address before saving. Fallback: if geocoding fails, raw coordinates (LAT/LNG) are saved as address text. Save uses retry logic (up to 3 attempts with exponential backoff). After save, the returned data is verified against what was sent. Console audit logs show exactly what was sent to Supabase (`[LOCATION]` prefix). The `current_location` field format is `"DESCRIPTION | ADDRESS"` where ADDRESS is always populated.
