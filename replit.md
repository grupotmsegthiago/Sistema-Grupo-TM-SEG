...
-   **Franchise Financial Calculation:** Implements strict franchise rules for extra KM and hours, with tolerance-based auditing and prioritizing user-saved values. Critical rule: `isFixedDistanceClientRule` / `isFixedDistanceProviderRule` must ONLY activate for truly fixed-distance tables. Tables with "ATÉ", "ATE ", "ATE´", "ATE'", or "FAIXA" are FRANCHISE tables and must NEVER be treated as fixed distance.
