...
-   **Franchise Financial Calculation:** `financialUtils.ts` enforces strict franchise rules: KM_Extra = Max(0, Real_KM - Franchise_KM), Hour_Extra = Max(0, Real_Hours - Franchise_Hours). Special rule: ≤200km AND ≤2h = minimum activation only (no extras). `auditMissionFinancials()` compares stored vs calculated values with R$5 tolerance, but SKIPS flagging if `billing_verified_by` is set (user-saved values have absolute priority). MissionCard shows "Divergente" amber badge only for unverified missions. MissionFinancialModal respects both `billing_approved` and `billing_verified_by` to preserve user-saved values on reopen (no recalculation).
-   **Client/Provider List Sorting:** ClientList orders by `trading_name` ascending (alphabetical, nome fantasia). ProviderList orders by `trading_name || name` client-side with `localeCompare('pt-BR')` for proper alphabetical sorting. Both display `trading_name || name` (nome fantasia with fallback to razão social).
...
-   **Profile Permissions Management:** `ProfileForm.tsx` uses `NAV_ITEMS` from `constants.ts` to display all system permissions. Features: auto-expand all modules on load, "Selecionar Todos"/"Desmarcar Todos" bulk toggle, progress bar showing permission count, "Completo"/"Parcial" badges per module. `NAV_ITEMS` includes all screens: Dashboard, Monitoramento, Rede de Apoio, Chatbot IA, Relatórios, Financeiro (9 sub-items), Cliente (6 sub-items incl. Propostas Comerciais), Fornecedor (6 sub-items), Configurações (6 sub-items). Sidebar `hasAccess` enforces role-based visibility.
