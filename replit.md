...
-   **Operational Report (AI):** `MissionOperationalReport.tsx` generates professional operational reports using Gemini AI. Each OS has a red briefcase button. The modal provides fields for operation, agent, address, and field observations. Supports photo uploads (facade, drone view, details) with dynamic slots. Gemini transforms simple notes into technical corporate security language. Output is a branded PDF with TMSEG logo (and CEVA logo for CEVA clients), sections: General Info, Field Findings, Security Analysis, Operational Chronology, Conclusions. Uses `html2canvas` + `jsPDF` for PDF export.
