...
-   **Patch realtime direcionado do MissionTable (Task #128):** Em vez de recarregar a lista inteira a cada evento realtime, `components/MissionTable.tsx` aplica o evento em UMA OS direto no estado (INSERT/UPDATE patcham uma linha enriquecida via `lookupMapsRef`; DELETE filtra). Os dados derivados (contadores de solicitação/acidente e mapas auxiliares de aprovação/evidência/logs/DHL/pedágio) são reconciliados de forma leve via `refreshDerivedData` (debounce ~900ms), versionado por `derivedReqIdRef` para descartar respostas fora de ordem. Patch direcionado só vale para acesso total; visão restrita/comercial usa recarga total com debounce. Guarda de startup (`initialFetchDoneRef`) evita aplicar patch antes do snapshot inicial. O patch só roda para usuários com acesso total — restritos/comercial não têm RLS e dependem de filtro server-side.
-   **Número grande acompanha a memória de cálculo (Task #133):** No `MissionFinancialModal.tsx`, o número grande editável (VALOR FINAL do cliente e do fornecedor) é, por padrão, uma cópia fiel do total da memória de cálculo, definido como `serviceTotal` (base + km + hora + IBL) + pedágio do input editável — exatamente a expressão exibida na linha "= R$ x" do breakdown (usar `serviceTotal + parseNumber(toll)`, NÃO `client.total`/`provider.total`, que somam o pedágio calculado e não o editado). O `useEffect` de autofill sincroniza o número grande com esse total mesmo quando já há valor salvo no banco (`dbValuesLoadedRef`), corrigindo valores antigos "presos"/divergentes. NÃO sincroniza quando: (a) há edição manual de propósito (`userManuallyEditedRef`, ligado no load quando há `revenue_edit_reason`/`cost_edit_reason` ou `billing_verified_by`) — preserva o override da diretoria e o aviso âmbar; (b) está salvando (`isSavingRef`); (c) a OS está travada/aprovada (`isEffectivelyLocked`) — o snapshot financeiro congelado nunca é tocado. Não reintroduzir o antigo auto-sync que apagava edições manuais: o guard `!userManuallyEditedRef.current` é o que distingue "valor proposital" de "valor velho".
-   **Primeiro render instantâneo do MissionTable:** O `isLoading` da tabela só bloqueia o necessário para desenhar as linhas: a busca das missões e o enriquecimento de `vehicles`/`client_vehicles` (placas dos cards). Assim que `setAllMissions(mapped)` roda, define-se `initialFetchDoneRef = true` e `setIsLoading(false)` imediatamente; só então os telefones de agente (botões de WhatsApp) e o `refreshDerivedData` (selos/contadores) são disparados em segundo plano (`void ...().catch`). Nunca mover a varredura de `agents` (telefones) nem os mapas derivados (aprovação/evidência/logs/DHL/pedágio) de volta para o caminho bloqueante — eles fariam a tela "Carregando..." esperar dezenas de consultas desnecessárias para desenhar a lista.
