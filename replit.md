...
-   **Intelligent Geographic Positioning (Reverse Geocoding):** Full reverse geocoding pipeline across the system. When a Google Maps link or GPS coordinates are captured, the system uses Google Maps Geocoder API to resolve the full textual address (Rua, Número, Bairro, Cidade, Estado). The `resolveLocationDisplay()` utility in `lib/utils.ts` detects URLs, coordinate fallbacks (`LAT/LNG`), and raw links — returning a `needsGeocode` flag + extracted coords. `MissionCard` does live async reverse geocoding with an in-memory cache (`geocodeCache`) and a `waitForGoogleMaps()` polling mechanism (up to 10s). `MissionStatusModal` does the same via `useEffect` + Google Geocoder. `UpdateMissionModal.reverseGeocode()` includes wait-for-maps polling, extracts street, number, neighborhood, city, state. On save, the system detects if `currentLocationName` contains coordinates or URLs and re-geocodes before writing. Auto-enrichment on modal open: if `current_location` in DB contains a URL, coords fallback, or is empty, the system resolves and saves the textual address back to the DB. The `current_location` field format is `"DESCRIPTION | ADDRESS"` where ADDRESS should always be a textual address (coordinates only as last resort). Save uses retry logic (up to 3 attempts with exponential backoff). Console audit logs with `[LOCATION]` prefix.
...
-   **Provider Lowest Cost Selection:** After initial table matching by region/distance, the system calculates the total cost (base + extra km + extra hour) for ALL candidate provider tables and automatically selects the one with the lowest total cost. This ensures optimal provider payment — e.g., a 100km table + extras may be cheaper than a 200km table base. Manual table overrides bypass this optimization. The detection log shows the comparison (e.g., "Menor Custo: ATÉ 100KM (R$634) vs ATÉ 200KM (R$800)"). The optimizer excludes CANCELADA tables from candidates for non-cancelled missions. The CEVA Jundiaí 200KM override was removed from the provider side (it only applies to CLIENT table selection now) to prevent forcing expensive tables when cheaper ones cover the distance.
