...
let RhWarnings: typeof import('../components/rh/RhWarnings').default;

before(async () => {
  React = await import('react');
  ({ createRoot } = await import('react-dom/client'));
  ({ default: RhWarnings } = await import('../components/rh/RhWarnings'));
});
  ...
  return {
    ...
    async render() {
      await React.act(async () => {
        root.render(React.createElement(RhWarnings, {
...
async function changeValue(element: HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement, value: string) {
...
test('lista, cria, edita e remove preservando tabela, busca e refresh', async () => {
  let listCalls = 0;
  let created: any;
  let updated: any;
  let removed = '';
  ...
  const services = {
    ...
    create: async (input: any) => {
      created = input;
      return { id: 'warn-new', employee_id: input.employeeId, ...input };
    },
  ...
  assert.match(mounted.container.textContent || '', /Advertências/);
  assert.match(mounted.container.textContent || '', /Verbal/);
  ...
  await changeValue(
    mounted.container.querySelector('select[name="warningType"]') as HTMLSelectElement,
    'Escrita',
  );
  await click(buttonByText(mounted.container, 'Salvar'));
  assert.equal(created, undefined);
  assert.ok(notifications.includes('error:Motivo é obrigatório'));

  await changeValue(
    mounted.container.querySelector('textarea[name="reason"]') as HTMLTextAreaElement,
    'Falta injustificada',
  );
  await click(buttonByText(mounted.container, 'Salvar'));
  assert.ok(created);
  assert.equal(created.warningType, 'Escrita');
  assert.equal(created.reason, 'Falta injustificada');
  assert.equal('updated_by' in created, false);
  ...
  assert.equal(updated.input.warningType, existing.warning_type);
...
test('leitura mantém lista vazia e escritas falhas nunca exibem sucesso', async () => {
  ...
  await click(buttonByText(mounted.container, 'Salvar'));
  assert.deepEqual(notifications, ['error:Falha ao operar advertências']);
