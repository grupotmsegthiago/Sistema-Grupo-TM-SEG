...
describe('Pedido de análise de OS', () => {
  ...
  it('UI e rotas integradas', () => {
    ...
    assert.match(fs.readFileSync('lib/screenAccess.ts', 'utf8'), /os-analysis-pending/);
    assert.match(fs.readFileSync('lib/screenAccess.ts', 'utf8'), /canViewOsAnalysisPendencies/);
    ...
    const pendingPage = fs.readFileSync('components/OsAnalysisPendingPage.tsx', 'utf8');
    assert.match(pendingPage, /\/api\/os-analysis\?/);
    assert.match(pendingPage, /from 'react'/);
    assert.match(pendingPage, /useMemo/);
    assert.match(pendingPage, /parseJsonResponse/);
    assert.match(pendingPage, /Array\.isArray\(data\.items\)/);
    assert.match(pendingPage, /os-analysis-load-error/);
