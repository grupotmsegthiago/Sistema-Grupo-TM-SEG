...
const TABLE = 'rh_employee_documents';
const POLICY = 'Allow all for rh_employee_documents';
const FORWARD = 'migrations/2026_08_31_fase4_rls_rh_employee_documents.sql';
const ROLLBACK =
...
describe('Fase 4 RH RLS — preparação rh_employee_documents', () => {
  it('mantém zero consumidor frontend runtime direto', () => {
    const direct = [
      ...walkRuntimeFiles('components'),
      ...walkRuntimeFiles('lib'),
    ].filter((file) =>
      /from\(['"]rh_employee_documents['"]\)/.test(read(file)),
    );
    ...
    const component = read('components/rh/RhEmployeeDocuments.tsx');
    const client = read('lib/rh/employeeDocumentsClient.ts');
  ...
  it('preserva API autenticada e service_role backend fail-closed', () => {
    const handler = read('api/rh-employee-documents.ts');
    const access = read('lib/rh/rhApiAccess.ts');
    const core = read('lib/rh/employeeDocumentsApiCore.ts');

    assert.match(handler, /authorizeRhApiRequest/);
    assert.match(handler, /createRhServiceRoleClient/);
    assert.match(handler, /\['GET', 'POST', 'DELETE'\]/);
  ...
  it('forward é exclusivo, idempotente e sem policy substituta', () => {
    const sql = read(FORWARD);
    assert.deepEqual(referencedPublicTables(sql), [TABLE]);
    assert.match(sql, /IF policy_total = 0 THEN\s+RETURN;/);
    assert.match(
      sql,
      /DROP POLICY IF EXISTS "Allow all for rh_employee_documents"\s+ON public\.rh_employee_documents/,
    );
    assert.doesNotMatch(sql, /\bCREATE\s+POLICY\b/i);
    assertNoDataOrSchemaChanges(sql);
  });

  it('forward aborta em qualquer drift antes do DROP', () => {
    const sql = read(FORWARD);
    ...
    assert.match(sql, /rls_enabled IS DISTINCT FROM true/);
    assert.match(sql, /force_rls IS DISTINCT FROM false/);
    assert.match(sql, /policyname = 'Allow all for rh_employee_documents'/);
  ...
  it('rollback é exclusivo, idempotente e restaura exatamente a policy anterior', () => {
    ...
    assert.match(sql, /CREATE POLICY "Allow all for rh_employee_documents"/);
  ...
  it('scripts globais não recriam policy e histórico permanece imutável', () => {
    ...
    assert.match(historical, /'rh_employee_documents'/);
    assert.match(
      historical,
  ...
  it('Realtime registrado não é dependência funcional dos documentos', () => {
    ...
    const listeners = runtime.filter((file) =>
      /supabase:rh_employee_documents(?::realtime)?/.test(read(file)),
    );

    assert.match(realtime, /rh_employee_documents:\s*\[\]/);
    assert.deepEqual(listeners, []);
    assert.doesNotMatch(
      component,
      /useRealtimeRefresh|supabase:rh_employee_documents/,
    );
  });
});
